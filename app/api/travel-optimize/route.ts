...
function fixTimeRange(start: string, end: string) {
  if (start === end) {
    ...
    return `${start} - ${String(newH).padStart(2, "0")}:${String(newM).padStart(
      2,
      "0"
    )}`;
  }
...
export async function POST(req: Request) {
  try {
    ...
    const result = await resOpt.json();

    // 5️⃣ Sprawdź wynik lub fallback
    if (!result.routes || result.routes.length === 0) {
      console.warn("⚠️ Brak wyniku optymalizacji — fallback na prostą kolejność");
      validOrders.sort((a, b) =>
        a.address.localeCompare(b.address, "pl", { sensitivity: "base" })
      );
    }
